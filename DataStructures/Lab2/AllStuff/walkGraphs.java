...
//                            "Unrecognized input: " + line);
    ...
    public static void getAllPaths(Integer start, Integer current,
                                     ...
                                     PathsString paths, String outputFile)
    {
        ...
        {
            if(graph.getAdjList()[current-1][i-1]) // if i is neighbor of current
            {
                if(i == dest && i == start) // include loops to start
                {
                    int j = 0;
                    ...
                    currentPath[j] = i; // add current node to path
                    paths.append(getCurrentPath(currentPath));
                    currentPath[j] = null; // cleanup
                }
                // if we have not visited i yet
                if(!hasVisited[i-1])
                {
                    int j = 0;
                    while(currentPath[j] != null)
                    ...
                    currentPath[j] = i; // add neighbor to current path

                    getAllPaths(start, i, dest, hasVisited, currentPath,
                            graph, paths, outputFile); // recursively solve
